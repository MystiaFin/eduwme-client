...
} from "./validators/courseBatch.validators.js";
import {
  courseSchema,
  courseUpdateSchema,
        ...
        if (exerciseExists) {
          const exerciseStatus =
            user.courseBatchesProgress[batchIndex].courses[courseIndex]
              .exercises[exerciseIndex].status;
 ...
 * GET /getCourses
 */
app.get("/getCourses", async (req: Request, res: Response): Promise<void> => {
  try {
    const pageSize = Number(req.query.page_size) || 10;
    const page = Number(req.query.page) || 1;
 ...
 * GET /getExercises
 */
app.get("/getExercises", async (req: Request, res: Response): Promise<void> => {
  try {
    const pageSize = Number(req.query.page_size) || 10;
    const page = Number(req.query.page) || 1;
    const search = typeof req.query.search === "string" ? req.query.search : "";
...
app.delete(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      existingCourse.exerciseBatchList =
        existingCourse.exerciseBatchList.filter(
